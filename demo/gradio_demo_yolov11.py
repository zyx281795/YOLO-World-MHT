...
def parse_args():
    parser = argparse.ArgumentParser(description='YOLO-World YOLOv11 Demo')
...
def export_model(runner,
                 ...
                 score_thr,
                 nms_thr,
                 model_path='./work_dirs/yolo_world_yolov11.onnx'):
...
def demo(runner, args):
    with gr.Blocks(title="YOLO-World + YOLOv11") as demo:
        with gr.Row():
            gr.Markdown('<h1><center>🚀 YOLO-World + YOLOv11: Real-Time Open-Vocabulary '
                        'Object Detector</center></h1>')
        
        # YOLOv11 feature highlight
        with gr.Row():
            gr.Markdown("""
            ### 🆕 YOLOv11 Improvements:
            - **C3k2 blocks**: More efficient than YOLOv8's C2f blocks
            - **C2PSA attention**: Enhanced spatial awareness mechanism  
        ...
        with gr.Row():
            with gr.Column(scale=0.3):
                with gr.Row():
                    image = gr.Image(type='pil', label='輸入圖片')
        ...
        demo.launch(server_name='0.0.0.0',
                    server_port=8082)  # Different port from YOLOv8 version
...
if __name__ == '__main__':
    ...
    # Enable YOLOv11 components import
    ...
    print("🚀 YOLOv11 YOLO-World Demo is ready!")
    print(f"📊 Model backbone: {type(runner.model.backbone).__name__}")
